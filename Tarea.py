...
class Operaciones:
     ...
     #En un estacionamiento el monto a pagar se calcula multiplicando el número de horas que permaneció 
     #el automóvil dentro del estacionamiento por 4Bs y se incrementa esta cantidad en 2.50 Bs por cada media hora
     #adicional. Ahora se desea que usted elabore un algoritmo que a partir de la hora de entrada y la hora de salida
     #de un vehículo (las mismas corresponden a un mismo día) calcule el monto a pagar por el dueño del vehículo.
     #La entrada vendrá dada por dos enteros positivos el primero representa las horas y el segundo los minutos, además 
     #por último se debe leer un carácter (A o P) que indica si la hora es AM o PM.
     def estacionamiento(self):
          d = [0, 0, "", 0, 0, ""]
          d1 = [0]*2
          pbs = ["Hora de ingreso", "Tiempo que se excede de entrada", 2, "Hora de salida", "Tiempo que se excede de salida", 5]
          ct = 0

          for i in pbs:
               if (ct != 2 or ct != 5):
                if (i != 2 and i != 5):
                    d[ct] = int(input("Ingrese {}: ".format(i)))
               ct += 1
               if ct == 2 or ct == 5:
                    c = input("¿La hora que ingresó es AM o PM?: ")
                    d[(pbs[ct])] = c

          d1[0] = (d[0] * 3600) + (d[1] * 60)
          d1[1] = (d[3] * 3600) + (d[4] * 60)
